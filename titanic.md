...
이 그림에서 가장 합리적인 선은 빨간색일 것! 
<br/>
BUT 로지스틱으로 빨간색 선을 그리기 어려움. 오차를 기반으로 하기 때문에 오차 작아지는 쪽으로 그렸을 때 빨간색 선이 그려질 수 없음.
<br/>
...
### 분류 문제에서의 SVM
![prac5](./image/prac5.png)
- 여백 : 주어진 데이터가 오류를 발생시키지 않고 움직일 수 있는 최대 공간
- 여백이 크다 = 새로운 데이터가 들어왔을 때 오류를 발생시키지 않도록 확보된 공간이 넓다
    - 다시 말해 여백이 크다는 것은 일반화 성능이 좋다는 것

### 회귀 문제에서의 SVM
![prac7](./image/prac7.png)
- 기본 가정 : 데이터들이 초평면으로부터 ϵ>0 범위에 있다
- 데이터들이 초록색 직선으로부터 ϵ범위 (양쪽 점선 사이)에 있는 상황이고, 입력 데이터는 1차원이므로 x축 상에서 왔다갔다할 것
- 여백 : 양 점선 사이의 수평 거리
<br/>

![prac6](./image/prac6.png)
- 여백(빨간 선)을 보면 왼쪽이 오른쪽보다 더 큼
- 분류 문제에서와 마찬가지로 SVM은 일반화 관점에서 여백이 큰 왼쪽 직선을 선호
...
- 여백이란?
    - 결정 경계와 가장 가까운 데이터 포인트 간의 거리로, 주어진 데이터가 오류를 발생시키지 않고 움직일 수 있는 최대 공간 (데이터 포인트들이 결정 경계에서 멀리 떨어져 있을수록 좋음)
    - 마진이 클수록 일반화 성능이 좋음! 
...
- *비선형 SVM* : 곡선이나 복잡한 형태의 경계를 만듬
    - 데이터가 선형적으로 나뉘지 않을 때
    - 원래 공간에서는 곡선 형태로 나뉘어야 하는 데이터를    "고차원 공간"에서 직선으로 변환하여 분류
